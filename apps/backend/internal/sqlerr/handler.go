...
func formatUserFriendlyMessage(sqlErr *Error) string {
	...
	case UniqueViolation:
		return fmt.Sprintf(
			"A %s with this identifier already exists",
			entityName,
		)
	...
	case CheckViolation:
		...
		if fieldName != "" {
			return fmt.Sprintf(
				"The %s value does not meet required conditions",
				fieldName,
			)
		}
...
func HandleError(err error) error {
	...
	if errors.As(err, &pgerr) {
		...
		case UniqueViolation:
			...
			if columnName != "" {
				userMessage = strings.ReplaceAll(
					userMessage,
					"identifier",
					humanizeText(columnName),
				)
			}
	...
	case errors.Is(err, pgx.ErrNoRows), errors.Is(err, sql.ErrNoRows):
		...
		if strings.Contains(errMsg, tablePrefix) {
			table := strings.Split(
				strings.Split(errMsg, tablePrefix)[1],
				":",
			)[0]
